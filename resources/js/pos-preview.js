...
function updateCartUI() {
    ...
    if (cart.length === 0) {
        if (emptyMsg) {
            ...
            listContainer.innerHTML = `
                <div class="cart-empty-state" id="cart-empty-msg" style="display:flex;">
                    <ion-icon name="basket-outline"></ion-icon>
                    <h3>Keranjang masih kosong</h3>
                    <p>Klik pada produk di sebelah kiri untuk menambahkan ke keranjang belanja.</p>
                </div>`;
        }
        if (countPill) countPill.textContent = '0 Item';
...
function processTransaction() {
    ...
    if (window.isLaravel) {
        ...
        .then(response => {
            if (!response.ok) {
                return response.json().then(err => { throw new Error(err.message || 'Transaksi gagal diproses oleh server.'); });
            }
            return response.json();
        })
        .then(responseData => {
            playSound('double-beep');
            const backendTrx = responseData.data;
            
            // Decrement physical mock database stocks in frontend memory to keep UI updated
            cart.forEach(item => {
            ...
            if (backendTrx.payment_method === 'kartu') methodLabel = 'Debit / Kredit';
            if (backendTrx.payment_method === 'qris') methodLabel = 'QRIS Mandiri';
            
            const dateObj = new Date(backendTrx.created_at);
            ...
            const transactionRecord = {
                invoice: backendTrx.invoice_no,
                time: dateStr,
                cashier: backendTrx.user ? backendTrx.user.name : 'Kasir',
                method: methodLabel,
                total: parseFloat(backendTrx.total_amount),
                paid: parseFloat(backendTrx.paid_amount),
                change: parseFloat(backendTrx.change_amount),
                items: backendTrx.details.map(d => ({
                    name: d.product ? d.product.name : 'Produk',
                    qty: d.qty,
        ...
        // Standalone Visual Preview Simulation Mode
        setTimeout(() => {
            playSound('double-beep');
...
function switchView(viewName) {
    // Dalam mode Laravel (MPA), sidebar menggunakan full-page navigation.
    // switchView hanya aktif pada standalone Vite SPA preview.
    if (window.isLaravel) return;
...
document.addEventListener('DOMContentLoaded', () => {
    // -------------------------------------------------------------------------
    // SINKRONISASI DATA LARAVEL: Jika berjalan di bawah Laravel (window.isLaravel),
    // timpa array produk & kategori internal JS dengan data riil dari database SQLite
    // yang diinjeksikan oleh cashier.blade.php via @json() helper.
    // -------------------------------------------------------------------------
    if (window.isLaravel) {
        if (window.laravelProducts && window.laravelProducts.length > 0) {
            // Hapus isi array dan ganti dengan data dari database
            products.length = 0;
            window.laravelProducts.forEach(p => products.push(p));
        }
        if (window.laravelCategories && window.laravelCategories.length > 0) {
            categories.length = 0;
            window.laravelCategories.forEach(c => categories.push(c));
        }
    }
    ...
    // Pada mode Laravel, sidebar menggunakan MPA (multi-page navigation),
    // masing-masing menu memicu navigasi halaman penuh — bukan SPA switchView.
    // Kita hanya daftarkan listener SPA pada mode standalone Vite preview.
    if (!window.isLaravel) {
        const navItems = [
        ...
        // Ensure default dashboard section is shown on standalone mode
