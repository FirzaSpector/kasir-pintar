...
@section('title', 'Kasir Utama - Kasir Pintar')
@section('header_title', 'Kasir Utama')
@section('header_subtitle', 'Kelola keranjang belanja dan transaksi penjualan langsung.')
        ...
        <div class="products-grid" id="products-container">
            @foreach($products as $product)
                <div class="product-card" 
                    ...
                    @if($product->stock < 10)
                        <div class="stock-badge warning">Sisa {{ $product->stock }}</div>
...
<script>
    ...
    window.laravelCategories = {!! json_encode($categories->map(function($c) { return ['name' => $c->name, 'slug' => $c->slug]; })) !!};
    window.laravelProducts = {!! json_encode($products->map(function($p) {
        return [
            'id' => $p->id,
