...
@section('title', 'Kasir Pintar - Manajemen Produk')
@section('header_title', 'Manajemen Produk')
@section('header_subtitle', 'Kelola data katalog produk, harga jual, dan stok persediaan.')
                ...
                <tbody>
                    @forelse($products as $product)
                        <tr>
                            ...
                            <td>{{ $product->name }}</td>
                            <td><span class="pill-method" style="background-color: var(--color-primary-light); color: var(--color-primary);">{{ $product->category->name ?? 'Tanpa Kategori' }}</span></td>
                            ...
                            <td>
                                ...
                                @elseif($product->stock < 10)
                                    <span class="pill-status warning" style="background-color: var(--color-warning-light); color: var(--color-warning); padding: 4px 8px; border-radius: 12px; font-size: 11px; font-weight: 700;">Sisa {{ $product->stock }}</span>
                                @else
                            ...
                            <td>
                                <div style="display: flex; gap: 8px;">
                                    <button class="btn btn-outline btn-small" style="padding: 6px 10px;" onclick="openLaravelProductModal({{ json_encode($product) }})">
                                        <ion-icon name="create-outline"></ion-icon>
                                    </button>
                                    <form action="{{ route('pos.products.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus produk ini?')">
                                        @csrf
                                        @method('DELETE')
...
<div class="modal-overlay" id="laravel-product-modal" style="display: none;">
    <div class="payment-modal" style="max-width: 500px;">
        ...
        <form id="laravel-product-form" action="{{ route('pos.products.store') }}" method="POST" style="padding: 24px;">
            @csrf
...
<script>
    function openLaravelProductModal(product = null) {
        ...
        if (product) {
            ...
            form.action = "{{ route('pos.products.store') }}";
